...
class Langganan(Base):
    ...
    email_status: Mapped[str | None] = mapped_column(String(50), nullable=True, index=True)  # Status Email (sent/pending/failed)
    last_email_sent: Mapped[datetime | None] = mapped_column(nullable=True, index=True)  # Terakhir kali Email dikirim
